...
public class AppointmentsAddModifyController {
    ...
    @FXML ComboBox<String> userIDComboBox;
    ...
    public void saveAppointments(ActionEvent event) throws IOException {
        ...
        if (!error) {
            try {
                ...
                String userSql = "SELECT * FROM users WHERE user_ID = '" + userIDComboBox.getValue() + "'";
                ...
                LocalDateTime localStart = LocalDateTime.of(startDatePicker.getValue(), startTimeComboBox.getValue());
                LocalDateTime localEnd = LocalDateTime.of(endDatePicker.getValue(), endTimeComboBox.getValue());
                ...
                if (currentTime.isBefore(targetTime))
                {
                    ...
                    localStart = localStart.plusHours(value);
                    localEnd = localEnd.plusHours(value);
                }
                ...
                if (currentTime.isAfter(targetTime))
                {
                    ...
                    localStart = localStart.minusHours(value);
                    localEnd = localEnd.minusHours(value);
                }
    ...
    private void setUserComboBox() {
        try {
            ...
            while (result.next()) {
                userIDComboBox.getItems().add(result.getString("User_ID"));
            }
        }
        catch (Exception ex) {
            System.out.println(ex.getMessage());
    ...
    private void setTextFields() {
        ...
        userIDComboBox.setValue(Integer.toString(selectedAppointment.getUserID()));
    }
    ...
    private boolean checkAppointments()
    {
        ...
        ObservableList<Appointments> checkList =FXCollections.observableArrayList();
        LocalDateTime startTime = LocalDateTime.of(startDatePicker.getValue(), startTimeComboBox.getValue());
        LocalDateTime endTime = LocalDateTime.of(endDatePicker.getValue(), endTimeComboBox.getValue());


        for (Appointments appointments : appointmentsList) {
            if (appointments.getContactName().equals(contactName)) {
                checkList.add(appointments);
            }
        }
        ...
        if (selectedAppointment != null)
        {
            ...
            {
                if(checkList.get(i).getAppointmentID() == Integer.parseInt(appointmentIDTextField.getText()))
                {
                    checkList.remove(i);
                }
            }
        ...
        for (Appointments appointments : checkList) {
            LocalDateTime timeCheck = appointments.getStart();
            do {
                if (timeCheck.equals(startTime) || timeCheck.equals(endTime)) {
                    if (!appointments.getStart().equals(endTime) && !appointments.getEnd().equals(startTime)) {
            ...
            while (!timeCheck.equals(appointments.getEnd().plusMinutes(30)));

            timeCheck = startTime;
            do {
                if (timeCheck.equals(appointments.getStart()) || timeCheck.equals(appointments.getEnd())) {
                    if (!appointments.getStart().equals(endTime) && !appointments.getEnd().equals(startTime)) {
                        errorLabel.setText(rb.getString("OverLappingAppointments"));
                        return true;
                    }
            ...
            while (!timeCheck.equals(endTime.plusMinutes(30)));
        }
        return false;
    }
    ...
    private boolean checkValuesOfUserInput()
    {
        ...
        {
            caughtErrors = caughtErrors + rb.getString("NoUserID") + "\n";
            error = true;
        }
